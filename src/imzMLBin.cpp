...
Rcpp::NumericVector imzMLBinReadAllMz(const char* ibdFname, Rcpp::List imzML)
   {
    Rcpp::DataFrame df;
    df=imzML["run"];
    bool continuous=imzML["continuous_mode"];
    Rcpp::String mz_dataType((const char*)imzML["mz_dataType"]);
    Rcpp::String int_dataType((const char*)imzML["int_dataType"]);
//    mz_dataType=imzML["mz_dataType"];
//    int_dataType=imzML["int_dataType"];
    int NPixels=df.nrows();
    Rcpp::IntegerVector mzLength=df["mzLength"];
    Rcpp::IntegerVector mzOffset=df["mzOffset"];
    int totalMassSize=0;
    for(int i=0; i<NPixels; i++) 
      totalMassSize+=mzLength[i];
    double *totalMass_p=0;
    int indexMz=0;
    int vez=1;
    printf("\nmassSize:%d nPixels:%d\n",totalMassSize, NPixels);
    try
     {
      totalMass_p=new double[totalMassSize];
      printf("Processing...(%%): 00 ");
      ImzMLBinRead myReader(ibdFname, NPixels, mz_dataType, int_dataType, continuous);
      for(int i=0; i<NPixels; i++)
       {
         if((float)i/(float)NPixels>vez*0.1) {printf("%d ", vez*10); vez++;}
         myReader.readMzData(mzOffset[i], mzLength[i], totalMass_p+indexMz);
//         if(i>NPixels-10) printf("%d %d\n", mzOffset[i], mzLength[i]);
         indexMz+=mzLength[i];
       }
       myReader.close();
       printf("100\n");
...
//    for(int i=totalMassSize-100; i<totalMassSize; i++) printf("%.5f\n", totalMass_p[i]);
  Rcpp::NumericVector mass(totalMassSize);
    for(int i=0; i<totalMassSize; i++)
      mass[i]=totalMass_p[i];
    
    if(totalMass_p) delete[]totalMass_p;
    return mass;
   }
...
Rcpp::NumericMatrix m_spc;
double *buffer;
printf("px:%d mz:%d  %d\n", (int)pixelIDs.length(), (int)massIndex.length(), massIndex[0]);
try
{
  //Allocate the spectra reading buffer
  buffer = new double[massIndex.length() * pixelIDs.length()];
  
  //Allocate the output matrix
  if( ((pixelIDs.length() * massIndex.length() * sizeof(double))/ (1024 * 1024 )) > SPECTRA_BUFFER_MB )
  {
    throw std::runtime_error("Error in Cload_imzMLImages(): loading data required too much memory.");
  }
  m_spc = Rcpp::NumericMatrix(pixelIDs.length(), massIndex.length());
  ...
  imzMLReader.setCommonMassAxis(commonMassAxis.length(), commonMassAxis.begin());
//  imzMLReader.setCommonMassAxis(massIndex.length(), (double*)&(commonMassAxis[massIndex[0]]));
...
//  void ImzMLBinRead::ReadSpectra(unsigned int numOfPixels, unsigned int *pixelIDs, unsigned int ionIndex, unsigned int ionCount, double *out, unsigned int number_of_threads, bool bUpdate_pixel_read_offsets)
//  imzMLReader.ReadSpectra(pixelIDs.length(), (unsigned int *) pixelIDs.begin(), 0, commonMassAxis.length(), buffer, number_of_threads); 
  imzMLReader.ReadSpectra(pixelIDs.length(), (unsigned int *)pixelIDs.begin(), massIndex[0], massIndex.length(), buffer, number_of_threads); 
  for(int i=0; i < pixelIDs.length(); i++)
  {
    for(int j = 0; j < massIndex.length(); j++)
    {
      m_spc(i,j) = buffer[j + i*massIndex.length()];
    }
  }
