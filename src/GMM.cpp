...
int Cgmm::gmm(GMM_STRUCT *gmm)
    ...
    for(int iy=0; iy<gmm->size; iy++) //accumulated in magnitude
        {
        sumaY+=gmm->y[iy];
        if(gmm->y[iy]>0.0 ) {countY++; valIndex=iy;}
        sumaYsqr+=gmm->y[iy]*gmm->y[iy];
        }
    if(countY==1) //solo un valor !=0 en array de magnitudes
    {
      gmm->params[0].weight=gmm->y[valIndex];
      gmm->params[0].mean  =gmm->x[valIndex];
      gmm->params[0].sigma =1e-5;
      gmm->yFactor=2.50662827*gmm->params[0].sigma;
      gmm->quality=1;
      if(sumaPxp_p) delete [] sumaPxp_p;
      if(pxk_p[0]) delete []pxk_p[0];
//  printf(".........%.4f/%.4f\n", gmm->params[0].mean, gmm->params[0].sigma);
...
while(true) //implementation of the EM algorithm (expectation-maximization)
    ...
    for(int ix=0; ix<gmm->size; ix++)
        ...
        for(int g=0; g<gmm->nGauss; g++)
            if(sumaPxk<1e-30) pxk_p[g][ix]=1e-30; else pxk_p[g][ix]/=sumaPxk;   //likelihood->probability normalization
        }
    ...
    for(int g=0; g<gmm->nGauss; g++)
        ...
        if(newWeight[g]<1e-30) 
        {
          newSigma[g]=gmm->limits[g].maxSigma;
        }
        else {
          newSigma[g]/=newWeight[g]; newSigma[g]=sqrt(newSigma[g]);
        }
        }

    //normalization of weights
    for(int g=0; g<gmm->nGauss; g++)
        if(sumaY>1e-30) newWeight[g]/=sumaY;
    ...
    if(sumaTotalPxk<1e-30)  sumaTotalPxk=1e-30; //casi cero
